...
public class JWTService {
	...
	public String gereraToken(final UserDetails userDetails) {
		...
		return JWT.create().withSubject(userDetails.getUsername()).withIssuer("puntoventa").withIssuedAt(now)
				.withExpiresAt(now.plusMillis(JWT_TOKEN_IAT.toMillis())).sign(hmac512);
	}
